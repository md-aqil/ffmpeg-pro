...
const Header = ({ 
  ...
  const getButtonProps = () => {
    // If no file is selected, button is disabled
    if (!selectedFile) {
      return {
        disabled: true,
        text: 'Convert',
        className: 'icon-btn save-btn disabled'
      };
    }
    
    // If conversion is in progress
    if (isConverting) {
      return {
        disabled: true,
        text: 'Converting...',
        className: 'icon-btn save-btn converting'
      };
    }
    
    // If conversion is complete, show download
    if (conversionResult) {
      return {
        disabled: false,
        text: 'Download',
        className: 'icon-btn save-btn download',
        onClick: onDownload
      };
    }
    
    // Default state - ready to convert
    return {
      disabled: false,
      text: 'Convert',
